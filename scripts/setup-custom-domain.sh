...
echo "    https://{slug}.${DOMAIN} → Container App (A record * → ${CA_STATIC_IP})"
echo "    ✅ Wildcard DNS configured via A record"
echo ""
echo "  ⚠️  Note: SSL certificates for api.${DOMAIN} are managed by Azure."
echo "     For wildcard subdomains (*.${DOMAIN}), the backend handles"
echo "     TLS termination or use the Container Apps managed cert."
