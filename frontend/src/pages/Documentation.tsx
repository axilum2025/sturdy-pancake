...
function CodeBlock({ code, language = 'bash' }: { code: string; language?: string }) {
  ...
  return (
    <div className="relative group rounded-xl bg-black/80 border border-t-border/20 overflow-hidden my-4">
...
export default function Documentation() {
  ...
  function EndpointTable({ endpoints }: { endpoints: ApiEndpoint[] }) {
    return (
      <div className="rounded-xl border border-t-border/20 overflow-hidden my-4">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-t-surface/50 border-b border-t-border/20">
              <th className="text-left px-4 py-2.5 text-t-text/50 font-medium text-xs uppercase tracking-wider">{t('docs.tableMethod')}</th>
              <th className="text-left px-4 py-2.5 text-t-text/50 font-medium text-xs uppercase tracking-wider">{t('docs.tablePath')}</th>
              <th className="text-left px-4 py-2.5 text-t-text/50 font-medium text-xs uppercase tracking-wider">{t('docs.tableDescription')}</th>
              <th className="text-left px-4 py-2.5 text-t-text/50 font-medium text-xs uppercase tracking-wider">{t('docs.tableAuth')}</th>
            </tr>
          </thead>
          <tbody>
            {endpoints.map((ep, i) => (
              <tr key={i} className="border-b border-t-border/10 hover:bg-t-surface/30 transition-colors">
                ...
                <td className="px-4 py-2.5 font-mono text-xs text-t-text/70">{ep.path}</td>
                <td className="px-4 py-2.5 text-t-text/60">{ep.description}</td>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 flex gap-8">
        ...
        <nav className="hidden lg:block w-56 flex-shrink-0">
          <div className="sticky top-24 space-y-1">
            <p className="text-xs font-semibold text-t-text/40 uppercase tracking-wider mb-3 px-3">{t('docs.sidebarTitle')}</p>
            {sections.map((s) => (
              <button
                key={s.id}
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all ${
                  activeSection === s.id
                    ...
                    : 'text-t-text/50 hover:text-t-text/80 hover:bg-t-surface/50'
                }`}
              >
        ...
        <div className="lg:hidden w-full mb-6">
          <select
            ...
            className="w-full px-4 py-2.5 rounded-xl bg-t-surface border border-t-border/30 text-sm"
          >
            {sections.map((s) => (
        ...
        <main className="flex-1 min-w-0">
          ...
          {activeSection === 'quickstart' && (
            <div>
              <h1 className="text-3xl font-bold mb-2">{t('docs.quickstartTitle')}</h1>
              <p className="text-t-text/60 mb-8 text-lg">{t('docs.quickstartSubtitle')}</p>

              <div className="space-y-8">
                <div>
                  <h3 className="text-lg font-semibold mb-2 flex items-center gap-2">
                    <span className="text-primary">1.</span> {t('docs.step1Title')}
                  </h3>
                  <p className="text-t-text/60 mb-3">{t('docs.step1Desc')}</p>
  ...
  -d '{"email": "you@example.com", "password": "yourpassword"}'`} />
                ...
                <div>
                  <h3 className="text-lg font-semibold mb-2 flex items-center gap-2">
                    <span className="text-primary">2.</span> {t('docs.step2Title')}
                  </h3>
                  <p className="text-t-text/60 mb-3">{t('docs.step2Desc')}</p>
  ...
  -d '{
    ...
    "config": {
      ...
      "tools": []
                ...
                <div>
                  <h3 className="text-lg font-semibold mb-2 flex items-center gap-2">
                    <span className="text-primary">3.</span> {t('docs.step3Title')}
                  </h3>
                  <p className="text-t-text/60 mb-3">{t('docs.step3Desc')}</p>
  ...
  -d '{"messages": [{"role": "user", "content": "Hello!"}]}'`} />
                ...
                <div>
                  <h3 className="text-lg font-semibold mb-2 flex items-center gap-2">
                    <span className="text-primary">4.</span> {t('docs.step4Title')}
                  </h3>
                  <p className="text-t-text/60 mb-3">{t('docs.step4Desc')}</p>
...
curl -X POST https://your-api.com/api/store/publish \\
  ...
  -d '{"agentId": "AGENT_ID", "name": "My Agent", "description": "..."}'`} />
              ...
              <div className="mt-10 p-5 rounded-2xl bg-primary/5 border border-primary/20">
                ...
                <p className="text-sm text-t-text/60 mb-3">{t('docs.baseUrlDesc')}</p>
          ...
          {activeSection === 'auth' && (
            <div>
              <h1 className="text-3xl font-bold mb-2">{t('docs.authTitle')}</h1>
              <p className="text-t-text/60 mb-6">{t('docs.authDesc')}</p>
              ...
              <h3 className="text-lg font-semibold mt-8 mb-3">{t('docs.authExampleTitle')}</h3>
...
curl -X GET /api/agents \\
  -H "Authorization: Bearer eyJhbG..."`} />
          ...
          {activeSection === 'agents' && (
            <div>
              <h1 className="text-3xl font-bold mb-2">{t('docs.agentsTitle')}</h1>
              <p className="text-t-text/60 mb-6">{t('docs.agentsDesc')}</p>
              <EndpointTable endpoints={agentEndpoints} />

              <h3 className="text-lg font-semibold mt-8 mb-3">{t('docs.agentsConfigTitle')}</h3>
  ...
  "knowledgeBase": ["doc-id-1", "doc-id-2"]
          ...
          {activeSection === 'knowledge' && (
            <div>
              <h1 className="text-3xl font-bold mb-2">{t('docs.knowledgeTitle')}</h1>
              <p className="text-t-text/60 mb-6">{t('docs.knowledgeDesc')}</p>
              ...
              <h3 className="text-lg font-semibold mt-8 mb-3">{t('docs.knowledgeFormatsTitle')}</h3>
          ...
          {activeSection === 'store' && (
            <div>
              <h1 className="text-3xl font-bold mb-2">{t('docs.storeTitle')}</h1>
              <p className="text-t-text/60 mb-6">{t('docs.storeDesc')}</p>
          ...
          {activeSection === 'api-keys' && (
            <div>
              <h1 className="text-3xl font-bold mb-2">{t('docs.apiKeysTitle')}</h1>
              <p className="text-t-text/60 mb-6">{t('docs.apiKeysDesc')}</p>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-t-surface/30 border border-t-border/20">
                    <MethodBadge method={ep.method} />
                    <code className="text-xs font-mono text-t-text/70">{ep.path}</code>
                    <span className="text-sm text-t-text/50 ml-auto">{ep.description}</span>
                  </div>
                ))}
              </div>
...
# ⚠️ The full key is only shown once!`} />
          ...
          {activeSection === 'webhooks' && (
            <div>
              <h1 className="text-3xl font-bold mb-2">{t('docs.webhooksTitle')}</h1>
              <p className="text-t-text/60 mb-6">{t('docs.webhooksDesc')}</p>
              ...
              <h3 className="text-lg font-semibold mt-8 mb-3">{t('docs.webhooksEventsTitle')}</h3>
          ...
          {activeSection === 'public-api' && (
            <div>
              <h1 className="text-3xl font-bold mb-2">{t('docs.publicApiTitle')}</h1>
              <p className="text-t-text/60 mb-6">{t('docs.publicApiDesc')}</p>
...
curl -X POST /api/v1/agents/AGENT_ID/chat \\
  ...
  -d '{"messages": [{"role": "user", "content": "Hello!"}]}'`} />
          ...
          {activeSection === 'deploy' && (
            <div>
              <h1 className="text-3xl font-bold mb-2">{t('docs.deployTitle')}</h1>
              <p className="text-t-text/60 mb-6">{t('docs.deployDesc')}</p>
          ...
          {activeSection === 'mcp' && (
            <div>
              <h1 className="text-3xl font-bold mb-2">{t('docs.mcpTitle')}</h1>
              <p className="text-t-text/60 mb-6">{t('docs.mcpDesc')}</p>
          ...
          {activeSection === 'rate-limits' && (
            <div>
              <h1 className="text-3xl font-bold mb-2">{t('docs.rateLimitsTitle')}</h1>
              <p className="text-t-text/60 mb-6">{t('docs.rateLimitsDesc')}</p>
